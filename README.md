# AI Search Algorithms

Implementation of uninformed search algorithms applied to maze solving.
Built for an AI course — algorithms implemented from scratch in Python.

## Projects

### Maze Solver — BFS, DFS, and Iterative Deepening Search
Three agents solve a 10x10 grid maze using different search strategies.

**Algorithms implemented:**
- Breadth-First Search (BFS) — guaranteed optimal, explores level by level
- Depth-First Search (DFS) — with branch-and-bound pruning for optimality
- Iterative Deepening Search (IDS) — combines BFS optimality with DFS memory efficiency

**Output per agent:**
- Optimal solution path (exact steps: U/D/L/R)
- Number of nodes expanded during search

## How to Run

**Requirements:** Python 3.x (no external libraries needed)

**Run:**
```bash
python maze_search.py
```

## Key Concepts
Uninformed search · BFS · DFS · Iterative deepening · 
Optimal pathfinding · Node expansion · Cycle detection · Branch-and-bound pruning

## Course
CSC6013 — Artificial Intelligence
